...
def handler(event, context):
    ...
    try:
        ...
        old_artists: Set[str] = set(old_content.get('artistIds', []))  # ['artistNames'] 
        ...
        artists_to_keep: Set[str] = old_artists.intersection(new_artists)
        ...
        for artist_id in artists_to_keep:
            try:
                artist_index_table.delete_item(
                    Key={'artistId': genre, 'contentKey': content_key}
                )
            except Exception as e:
                print(f"Greška pri brisanju starog artist indexa ({genre}): {str(e)}")
